...
public class LocaleListEditor extends SettingsPreferenceFragment
    ...
    final String PREF_NAME = "addingonce";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mPreferences = getActivity().getSharedPreferences(PREF_NAME,this.getContext().MODE_PRIVATE);
    ...
    private void configureDragAndDrop(View view) {
        ...
        //called method for add indonesian language by default
        addlang();
    ...
    //this method will add indonesian language by default
    public void addlang(){
         mPreferences = getActivity().getSharedPreferences(PREF_NAME,this.getContext().MODE_PRIVATE);
        mEditor = mPreferences.edit();
        String addonece = mPreferences.getString("addonces", "");
        LocalePickerWithRegion.LocaleSelectedListener mListener = new LocaleListEditor();
        List<LocaleStore.LocaleInfo> results = new ArrayList<>(); 
        Locale locales = new Locale("in","ID");
         Log.v("tageds","dadsver1 : "+addonece);
       results.add(LocaleStore.getLocaleInfo(locales));
        for(int i = 0 ; i < results.size(); i++){
            Log.v("tageds","dads : "+results.get(i));
        if(addonece.equals("")){
            Log.v("tageds","dads : "+results.get(i));
            mEditor.putString("addonces", "True");
            mEditor.commit();
            mEditor.apply();
            mAdapter.addLocale(results.get(i));  
        }
